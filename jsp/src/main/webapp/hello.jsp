...
서블릿 클래스를 컴파일하여 객체로 생성한 후 요청 처리 메소드를 호출하여 요청에 대한 처리와 응답파일
을 동적으로 생성하여 클라이언트에게 응답 처리 --%>
<%-- => JSP 문서에 대한 서블릿 객체가 이미 존재할 경우 요청 처리 메소드만 호출하여 응답처리  --%>
<%-- => JSP 문서가 변경된 후 JSP 문서를 요청하면 JSP 문서를 서블릿 클래스로 변환해 객체 생성하고 
요청 처리 메소드를 호출하여 응답 처리 --%>
<%
	Date now=new Date();
...
<body>
	...
	, 지시어(Directive), 표준 액션 태그(Standard Action))</p>
	<hr>
