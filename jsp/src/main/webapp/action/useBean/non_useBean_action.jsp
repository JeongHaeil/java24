...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%-- 전달값(회원정보)을 반홚받아 내장객체의 속성값으로 저장하고 [non_useBean_display.jsp]
...
<%
	...
	//post 방식으로 요청하여 전달된 값 읽기 위한 문자형태 변경
	...
	*/
	...
	hewon.setAddres(address);
	/*
	//session 내장객체에 HewonBean 객체를 속성값으로 저장
	session.setAttribute("hewon", hewon);
	
	//리다이렉트 이동 : 클라이언트에게 URL 주소를 전달하여 클라이언트가 URL 주소의 JSP 문서를 
	//요청해 실행결과를 응답받아 출력 처리되도 록 페이지 이동
	// => 클라이언트 브라우저의 요청 URL 주소 변경 - 클라이언트를 사용해 JSP 문서 이동
	// => session 내장객체에 속성값을 저장하여 리다이렉트 이동되는 JSP 문서에서 속성값을 반환받아 
	...
	*/
	...
	// => 클라이언트 브라우저 요청 URL 주소 미변경 - 서버의 JSP 문서에서 이동 처리
	// => request 내장객체에 속성값을 저장하여 포워드 이동되는 JSP 문서에서 속성값을 반환받아
	// => 포워드 이동되는 JSP 문서에서는 request 내장객체의 속성값을 반환받은 후 삭제 처리 불필요
	
	//forward 태그 대신 request 객체로 메소드를 호출하여 포워드 이동 가능
	//request.getRequestDispatcher (String conmtextPath) : 매개변수로 전달받은  웹자원의 경로 컨텍스트가 저장된
	//RequestDispatcher 객체 반환하는 메소드
