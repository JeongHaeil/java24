...
//Runtime 클래스 : 프로그램 실행 관련 정보를 제공하기 위한 기능의 메소드
// => Runtime 클래스는 프로그램에 객체를 하나만 제공하는 싱글톤 클래스
public class RuntimeApp {
	public static void main(String[] args) {
		//Runtime 클래스의 생성자 은닉화 처리되어 있으므로 new 연산자로 생성자를 호출하여 객체생서 불가능
		...
		System.out.print("메모리 정리 전 사용량 = ");
		System.out.println(Runtime.getRuntime().totalMemory()-Runtime.getRuntime().freeMemory()+ " Byte");
		
		//Runtime.gc() : 메모리를 정리하는 프로그램을 실행하는 메소드
		
		// => System.gc() : 메모리를 정리하는 프로그램을 실행하는 정적 메소드
