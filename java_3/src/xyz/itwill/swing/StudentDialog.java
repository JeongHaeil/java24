...
public class StudentDialog extends JDialog {
	...
	public StudentDialog(JFrame frame,String title) {
		//부모클래스 (JDialog 클래스)의 JDialog(Frame owner, String title, boolean model) 생성자를
		//생성자를 super 키워드로 호출하여 JDialog 객체 생성
		// => owner 매개변수에는 부모창(JFrame 객체) 을 전달 받고 model 매개변수에는
		//부모창(JFrame 객체) 비활성화 여부를 설정하는 논리값(false : 부모창 활성화 - Modeless Dialog, 
		//  true : 부모창 비활성화 - Model Dialog) 을 전달받아 JDialog 객체 생성
		...
		}
		{
			...
			nameTF.setFont(new Font("굴림", Font.PLAIN, 20));
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						//=> Vector 객체가 JTabel 컴퍼넌트의 하나의 행으로 출력 
						...
						//DefaultTableModel.addRow(vector) : Vector 객체를 JTable
						//컴퍼넌트의 행으로 추가하는 메소드
						// => Vector 객체의 요소값이 행을 구성하는 열의 값
