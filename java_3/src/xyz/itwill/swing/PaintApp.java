...
//프레임에서 마우스 버튼을 클릭한 경우 클릭한 위치에 좌표값을 문자열로 출력하는 프로그램
// =>마우스 관련 이벤트는 MouseEnvet 클래스를 사용하여 표현
public class PaintApp extends JFrame{
	private static final long serialVersionUID = 1L;
	
	//마우스 버튼을 클릭한 좌표값 저장하기 위한 필드
	//=> 이벤트 처리 메소드에서 좌표값을 제공받아 필드에 저장하고 paint( )  메소드에서는 필드값 출력
	private int x,y; 
	...
	public PaintApp(String title) {
		...
		// => MouseAdapter 추상클래스를 상속 받은 익명의 클래스를 사용하여 객체 생성해 전달
		addMouseListener(new MouseAdapter() {
			//컴퍼넌트에서 마우스 버튼을 누른 경우 자동 호출되는 이벤트 처리 메소드
			//=> 이벤트 처리 메소드의 매개변수에는 이벤트 관련 정보가 저장된 Event 객체 전달받아
			//이벤트 처리 메소드에서 사용가능
			@Override
			public void mouseClicked(MouseEvent e) {
				...
				//MouseEvent.getX() : 마우스 버튼을 클릭한 y 좌표값을 반환하는 메소드
				y=e.getY();
	...
	public static void main(String[] args) {
		new PaintApp("PaintAPP");
	}
	
	//Window.paint(Graphics g) : 컨테이너(JFrame, JWindow, JDialog 등)그림을 출력하는 메소드
	// => 프로그램이 실행되어 컨테이너가 보여지는 경우 또는 아이콘에서 해제되어 컨테이너가
	//보여진느 경우, 컨테이너의 크기가 변경되는 경우 자동 호출되는 메소드
	// => Graphics 객체 : 컨테이너에 그림을 그리는 기능을 제공하는 객체
	//컨테이너에 원하는 그림을 그리기 위해 Window 클래스(JFrame)의 paint() 메소드 오버라이딩
	@Override
	public void paint(Graphics g) {
		//System.out.println("paint(Graphics g) 호출");
		//super.paint(g);
		
		//super 키워드로 부모클래스 숨겨진 메소드 호출
		//=>부모클래스의 paint() 메소드를 호출하여 컨테이너 초기화 처리
		super.paint(g);
		
		/*
		//Graphics 객체로 메소드를 호출하여 다양한 형태 그림 출력 가능
