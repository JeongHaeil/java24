...
//파일경로 : 플랫폼의 파일(디렉토리)의 위치를 표현하는 방법
//=> 절대경로 표현방법 , 상대경로 표현방법
// => 절대경로 표현 방법 : 최상위 디렉토리를 기준으로 파일의 위치를 표현하는 방법
// => 상대경로 표현 방법 : 현지 실행중인 프로그램의 작업 디렉토리를 기준으로 파일의
//위치를 표현하는 방법 - 이클립스에서는 프로그램이 작성된 프로젝트 디렉토리를 작업디렉토리 사용
public class FileApp {
	public static void main(String[] args) throws IOException {
		//Window 운영체제에서는 드라이브를 최상위 디렉토리로 사용하여 디렉토리와 파일을
		//구분 하기 위해 [ \ ] 기호사용
		// => Java 언어에서는 [ \ ] 기호를 사용해 회피문자를 표현하므로 디렉토리와 파일 구분
		//하기위에서는 [\\] 형태로 작성해야 [ \ ] 기호로 사용 가능 
		File fileOne=new File("c:\\data");
		
		//File.exists() : File 객체에 저장된 파일 경로에 파일(디렉토리)가 없는 경우 
		//[false]를 반환하고 파일(디렉토리)가 있는 경우 [true] 반환
		if(fileOne.exists()) {
			System.out.println("c:\\data 폴더가 이미 존재합니다");
		}else {
			//File.mkdir() : File 객체에 저장된 디렉토리 경로를 생성하는 메소드
			fileOne.mkdir();
			System.out.println("c:\\data 폴더를 생성 합니다");
		}
		System.out.println("====================================================");
		
		//File fileTwo=new File("c:\\data\\itwill.txt");
		//Windows 운영체제를 제외한 나머지 운영체제에서는 디렉토리와 파일을 [ / ] 기호를 사용해서 구분
		// => Java 언어에서는 Windows 운영체제를 사용해도 [ \ ] 기호 대신 [/] 기호 사용해 파일 구분 가능
		File fileTwo=new File("c:/data/itwill.txt");
		...
		if(fileTwo.exists()) {
			...
			System.out.println("c:\\data\\itwill.txt 파일 생성 합니다");
		}
		System.out.println("====================================================");
		...
		// => 파일이 없이 파일을 삭제하지 못한경우 [false] 반환 삭제한경우 [true]
		if(fileThree.exists()) {
			fileThree.delete();
		...
		if(fileFour.exists()) {
			...
			//File.getAbsolutePath : File 객체에 저장된 파일 경로를 절대결로의
			//문자열로 반환하는 메소드
		...
		//File.isDirectory : File 객체에 저장된 파일(디렉토리) 경로가 디렉토리인 경우 [true] 반환
		if(fileFive.isDirectory()) {
			
			//File.lisFiles : File 객체에 저장된 디렉토리 경로의 디렉토리에서 하위파일(디렉토리)
