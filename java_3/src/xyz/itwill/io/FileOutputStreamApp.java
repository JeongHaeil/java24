...
public class FileOutputStreamApp {
	public static void main(String[] args) throws IOException {
		System.out.println("[메세지] 키보드를 눌러 값을 입력해 주세여[프로그램 종료 : Crtl + Z ]");
		...
		// => 매개변수로 전달받은 파일이 있는 경우 기존 내용을 초기화하여 새로운 내용으로 저장되도록 처리
		
		//FileOutputStream out=new FileOutputStream("c:/data/byte.txt");
		
		//FileOutputStream(String name, boolean append) 생성자를 사용하여 FileOutputStream 객체
		...
		// 부모 : IOException  자식 : FileNotFoundException
		// IOException 예외 처리 가능
		int readByte;
		while(true) {
		...
		System.out.println("c:\\data\\byte.txt 파일을 확인해보세여");
	}
}
