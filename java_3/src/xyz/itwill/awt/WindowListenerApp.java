...
// =>프레임(Frame 객체) 에서는 WindowEvent 발생되므로 WindowListener 인터페이스 상속받은
...
public class WindowListenerApp extends Frame{
	...
	//Listener 인터페이스 상속받은 자식클래스(이벤트처리)는 Listener 인터페이스
	//모든 추상 메소드를 무조건 오버라이딩 선언
	// => 이벤트처리 명령이없는 불필요한 이벤트 처리 메소드도 오버라이딩 선언
