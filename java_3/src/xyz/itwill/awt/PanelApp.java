...
//Panel 클래스 : 컴퍼넌트를 배치하여 그룹화기 위한 컨테이너 - 종속적인 컨테이너
...
public class PanelApp extends Frame{
	...
	public PanelApp(String title) {
		...
		//Panel 객체의 기본 배치관리자는 FlowLayout로 설정
		...
		//TextField 클래스 : 한 줄의 문자열을 입출력 처리하기 위한 컴퍼넌트를 구현한 클래스
		...
		//Componebt.setFont(Font f) : 컴퍼넌트의 글자 관련 속성을 변경하는 메소드
		// => 컨테이너에 적용된 글자 관련 속성은 컨테이너에 종속된 컴퍼넌트에 동일하게 적용
		
		//Font 클래스 : 글자관련 속성을 저장한 객체 생성하기 위한 클래스
		//Font 객체는 클래스의 Font(String name, int style, int size) 생성자를 사용하여 객체 생성
		...
		textField.setFont(new Font(Font.SANS_SERIF, Font.BOLD+Font.ITALIC,20));
		...
		// => Color 객체는 Color 클래스의 상수필드로 사용가능
		textArea.setBackground(Color.lightGray);
		
		//Component.setEnbale(boolean b) : 컴퍼넌트에 활성화 및 비활성화를 변경할수있는 메소드
		//=> false : 컴퍼넌트 비활성화, true(기본) : 컴퍼넌트 활성
		
		red.setEnabled(true);
		
		//TextComponent.setFocusable(boolean b) : 텍스트 컴퍼넌트에 포커스(Focus) 제공 여부를 변경하는 메소드
		//텍스트 컴퍼넌트를 사용한 입력 처리 불가능 - 출력용도
