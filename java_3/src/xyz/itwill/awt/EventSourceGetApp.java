...
public class EventSourceGetApp extends Frame{
	...
	private Canvas canvas;//생성자에서 컴퍼넌트를 생성하여 필드 저장
	private Button red, green, blue, white;
		...
		public EventSourceGetApp(String title) {
			...
			pane1.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 20));
			...
			//컴퍼넌트에서 이벤트가 발생될 경우 이벤트 처리를 위한 객체 추가하여 이벤트 처리
			//메소드가 자동 호출되도록 이벤트 처리 객체 등록 처리
			// => 컴퍼넌트마다 이벤트를 처리하는 클래스를 다르게 작성
			// => 익명의 클래스로 이벤트 처리 객체를 생성해 이벤트 처리 되도록 등록 권장
			/*
			...
			*/
			//모든 버튼에서 발생되는 이벤트를 하나의 에빈트 처리 클래스의 객체로 처리되도록 작성
			// => 이벤트 처리 메소드에서 이벤트가 발생된 컴퍼넌트로 비교하여 명령 선택실행
	...
	//모든 색상 버튼에서 발생된 이벤트를 처리하기위한  이벤트 처리 클래스
	public class ColorButtonEventHandle implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			// => 이벤트가 발생된 컴퍼넌트가 Object 클래스의 객체로 반환되므로 컴퍼넌트에
			//메소드를 호출하기 위해서는 명시적 객체 형변환 필요
