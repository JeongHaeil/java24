...
public class MenuBarApp extends Frame{
	...
	public MenuBarApp(String title) {
		...
		//MenuBar 클래스 : 프레임(Frame)에 메뉴바(MenuBar)를 배칠 할 수 있는 객체
		MenuBar menuBar=new MenuBar();
		
		//Menu 클래스 : 메뉴바(MenuItem 객체)을 메뉴(Menu)에 배치할 수 있는 객체를 생성하기 위한 클래스
		Menu file=new Menu("File");
		Menu help=new Menu("Help");
		
		//MenuItem 클래스 : 메뉴(Menu 객체)에 아이템을 배치할 수 이쓴 클래스
		//MenuShortcut 클래스 : 메뉴아이템의 단축키를 생성 및 저장하기 위한 클래스
		...
		MenuItem save=new MenuItem("Save", new MenuShortcut(KeyEvent.VK_1));
		...
		file.add(open);
		file.add(save);
		file.addSeparator(); // 메뉴 아이템을 구분하는 구분선을 배치
		file.add(exit);
		...
		//Fram.setMenuBar(MenuBar menuBar) : 프레임에 메뉴바를 변경하는 메소드
		setMenuBar(menuBar);
