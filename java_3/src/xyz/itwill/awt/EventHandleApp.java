...
//컴퍼넌트 에서는 다양한 이벤트가 발생될 수 있으며 이벤트 발생시 원하는 명령이 실행
//되도록 프로그램 작성 - 이벤트 처리 프로그램

//이벤트 처리 프로그램을 작성하는 방법
//1. 컴퍼넌트(컨테이너) 관련 클래스를 사용하여 UI 클래스 작성
// => UI 클래스는 기본적으로 Frame 클래스를 상속 받는다
// => UI 클래스에서 사용된 컴퍼넌트(컨테이너) 다양한 이벤트 발생 - 이벤트 소스(Event Source
// => 컴퍼넌트(컨테이너)에서 이벤트가 발생될 경우 이벤트 관련 정보가 저장된 XXXEvent 클래스로 객체가 자동 생성 
//ex) Button 컴퍼넌트 누른 경우 ActionEvent 클래스로 객체가 자동 생성
...
//이벤트 처리 클래스의 객체를 추가하는 메소드  
//이벤트가 발생되면 이벤트 처리 클래스의 객체로 이벤트 처리 메소드가 자동 호출
// => addXXXListener() 메소드를 호출하면 컴퍼넌트 이벤트를 감지하는 스레드가 자동 생성되어 컴퍼넌트에
//컴퍼넌트 이벤트가 발생되면 이벤트 처리 객체로 메소드 자동 호출
//이벤트 처리 프로그램은 기본적으로 다중 스레드 프로그램임

//EXit 버튼을 누른 경우 프로그램을 종료하는 이벤트 프로그램작성
// => 이벤트 처리 클래스의 메소드에서 UI 클래스의 컴퍼넌트(컨테이너) 사용이 불가능
...
//이벤트 처리 클래스 - Listener 인터페이스 상속
class ButtonEventHandel implements ActionListener{
