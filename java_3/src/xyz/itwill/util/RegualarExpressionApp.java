...
//사용하여 일정한 패턴의 문자열을 표현하기 위한 표현식 - 사용자 입력값 검증처리
...
\W : 영문자, 숫자, 특수문자(_)의 문자를 제외한 나모지 문자로 구성된 문자열을 의미
...
public class RegualarExpressionApp {
	public static void main(String[] args) {
		...
		//사용자로부터 [아이티]를 입력받아 원하는 패턴의 문자열인지 검사
		// => 아이디는 영문자로 시작되며 영문자,숫자,특수문자( _ ) 의 문자 조합으로 6~20범위의
		//문자로 구성된 패턴 문자열
		...
		//아이디 입렵값에 대한 패턴정보를 정규 표현식으로 작성하여 저장
		//String idReg="^[a-zA-Z][a-zA-Z0-9_]{5,19}$";
		String idReg="^[a-zA-Z]\\w{5,19}$";
		
		//Pattern 클래스 : 정규 표현식이 저장한 객체를 생성하기 위한 클래스
		//Pattern.matches(String regEx(정규표현식) , CharSequence input (입렵값) ) : 매개변수로 전달받은 정규 표현식과
		// 입력값의 패턴을 비교하여 다른 경우 [false]를 반환하고 같은 경우 [true] 반환을 하는 정적메소드
		...
		System.out.println("정상적인 아이디를 입력했습니다");
		
		
		//사용자로부터 [비밀번호]를 입력받아 원하는 패턴의 문자열인지 검사
		// => 아이디는 영문자로 시작되며 영문자,숫자,특수문자가 반드시 1번 이상 포함된 문자 조합으로 8~30 범위의
		//문자로 구성된 패턴 문자열
		...
		if(!Pattern.matches(passwordReg, password)) {
			System.out.println("[에러] 비밀번호는 9~30자의 영문자,숫자,특수문자를 반드시 사용하여 입력 가능합니다.");
			System.exit(0);
		}
			System.out.println("비밀번호 생성 완료했습니다.");
			
		*/
		
		//사용자로부터 [이메일]를 입력받아 원하는 패턴의 문자열인지 검사
		// => 이메일은[아이디@도메인]형식으로 구성되도록 패턴 설정처리
		...
		if(email==null || email.equals("")) { //사용자로부터 입력받은 값이 없는 경우
			System.out.println("[에러] 이메일를 입력해 주세요.");
			System.exit(0);
		}
		...
		if(!Pattern.matches(emailReg, email)) {
			System.out.println("[에러] 이메일은 [아이디@도메인] 형식으로 사용하여 입력 가능합니다.");
			System.exit(0);
		}
			System.out.println("사용할수있는 이메일입니다");
