...
<<<<<<< HEAD
//java.text 패키지 : 객체에 저장된 값을 원하는 형태의 문자열 변환하는 기능을 제공하는 
//Java 자료형이 작성된 패키지
...
public class SimpleDateFormatApp {
	public static void main(String[] args) {
		...
		//플렛폼 현재날짜와 시간이 저장된 Date 객체를 반환받아 저장
		Date now=new Date();
		System.out.println(now);
		
		//new 연산자로 SimpleDateFormat 클래스의 매개변수가 작성된 생성자를 호출하여 객체 생성
		// => SimpleDateFormat (String pattern) 생성자로 매개변수에 날짜와 시간 관련 패턴 정보를
		//전달받아 저장한 SimpleDateFormat 객체 생성
		//=> 패턴정보 : 패턴문자 (날짜와 시간을 표현하기 위한 문자)를 사용해 날짜와 시간을 표현한 문자열
		
		// => 패턴문자 : y(년), M(월), d(일), E(요일), a ( 오전 오후), h(시 : 12시기준)
		// H(시간 24시간기준), m(분),s(초)
		
		SimpleDateFormat dateFormat=new SimpleDateFormat("yyyy년 MM월 dd일 E요일");
		
		//SimpleDateFormat.format : 매개변수로 전달받은 Date 객체에 저장된 날짜와
		//시간을 SimpleDateFormat 객체에 저장된 패턴정보 문자열로 변환하여 반환하는 메소드
		String printDate=dateFormat.format(now);//Date 객체 >> String 객체
		
		System.out.println(printDate);
		
		//SimpleDateFormat.applyPattern : SimpleDateFormat 객체에 저장된
		//패턴정보를 매개변수로 전달받은 문자열로 변경하는 메소드
		...
		try {
			//SimpleDateFormat.parse : SimpleDateFormat 객체에 저장된 패턴정보와
			//동일한 형태의 문자열(String 객체)을 매개변수로 전달받아 Date 객체로 반환하는 메소드
			// => SimpleDateFormat 객체에 저장된 패턴정보와 매개변수로 전달받은 문자열의 패턴
			//일치 하지 않는 경우 ParseException 예외발생 - 일반예외 이므로 반드시 예외처리
			Date wantDate=dateFormat.parse(wantStr); //String >> Date 
			System.out.println(wantDate);
		} catch (ParseException e) {
			System.out.println("형식에 맞는 시간 입력");
		}
	}
}
=======
