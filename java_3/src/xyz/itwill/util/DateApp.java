...
public class DateApp {
	...
	public static void main(String[] args) {
		...
		//Date 객체가 저장된 참조변수를 출력하면 자동으로 toString() 호출
		System.out.println("현재 날짜와 시간 =  "+now); 
		
		//Date.toLocaleString() : Date 객체에 저장된 날짜와 시간을 플렛폼의 지역에 맞는 형태
		//문자열로 변환하여 반환하는 메소드 - @Deprecated 어노테이션 적용
		...
		//Date.getYear() : Date 객체에 저장된 날짜와 시간중 [년도]를 반환하는 메소드
		// => 1900년부터 1씩 증가된 정수값 반환
		
		//Date.getMonth() : Date 객체에 저장된 날짜와 시간중 [월]를 반환하는 메소드
		// = > 0(1월)~11(12월) 범위의 정수값
		
		//Date.getDate() : Date 객체에 저장된 날짜와 시간중 [일]를 반환하는 메소드
		
		//Date.getDay() : Date 객체에 저장된 날짜와 시간중 [요일]를 반환하는 메소드
		// =>0(일) ~ 6(토) 범위 정수값 반환
		...
		//Date.getTime() : Date 객체에 저장된 날짜와 시간을 시간값(TimsStamp)로 변환하여 반환하는 메소드
		long currentTime=now.getTime(); //long currentTime=System.currentTimeMillis()
		
		//new 연산자로 Date 클래스의 Date (int year, int month, int date) 생성자를 호출 객체생성
		
		//=> [ 2000년 1월 1일 0시 0분 0초] 의 날짜와 시간이 저장된 Date 객체
		Date wantDate=new Date(95, 1, 5);
