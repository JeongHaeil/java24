...
public class PenguinMoveApp extends JFrame{
	...
	//펭귄 이미지의 폭과 높이를 저장한 상수필드
	// => 폭과 높이로 사용하고 이벤트 처리 메소드에서 이동
	...
	public PenguinMoveApp(String title) {
		...
		//URL 클래스 : 리소스 파일(그림파일 , 소리파일, 동영상파일)의 파일 시스템 경로 또는
		//인터넷상에 존재하는 자원의 경로가 저장된 객체를 생성하기 위한 클래스
		
		//Object.getClass() : 현재 사용중인 클래스에  대한 정보가 저장된 class 객체를 반환하는 메소드
		//Class.getResoutce(String name) : 현자 사용중인 클래스의 파일 경로를 기존으로 리소스
		//파일을 제공받아 리소스 파일 정보가 저장된 URL 객체를 반환하는 메소드
		
		//ImageIcon.getImage() : ImageIcon 객체에 저장된 그림파일에서 이미지 저장된 Image
		//객체를 반환하는 메소드
		backImage=new ImageIcon(getClass().getResource("/images/back.jpg")).getImage();    
		
		//필드에 Image 객체를 요소값으로 저장할 수 있는 배열을 생성하여 저장
		penguins=new Image[3];
		
		//배열 요소에 펭귄 이미지 파일을 읽어 이미지 저장
		...
		//프레임에서 키보드 관련 이벤트가 발생될 경우 처리하기 위해 이벤트 처리 객체
		//객체를 익명의 클래스로 생성하여 등록
		addKeyListener(new KeyAdapter() {
			//키보드 버튼을 누르고 있는 상태인 경우 자동 호출되는 이벤트 처리
			@Override
			public void keyPressed(KeyEvent e) {
				//KeyEvent.getKeyCode() : 누르고있는 키보드 버튼의 코드값을 반환하는 메소드
