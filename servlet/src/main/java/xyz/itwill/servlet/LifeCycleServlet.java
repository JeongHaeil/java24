...
//메소드만 호출하여 실행 -기본 서블릿 객체 사용
// =>WAS 프로그램이 종료되면 WAS 프로그램에 의해 생성되어 사용된 서블릿 객체 소멸
...
public class LifeCycleServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		//(WAS 프로그램의 정보를 저장한 객체)반환하는 메소드 
		//ServletContext.getInitParameter(String name) : [web.xml] 파일의 context-param 엘리먼트에
		//제공된 값을 읽어와 반환하는 메소드
		//ServletConfig.getInitParameter(String name) : [web.xml] 파일의 init-param 엘리먼트에
		//제공된 값을 읽어와 반환하는 메소드
		name=config.getServletContext().getInitParameter("name");
	}
	
	
	//WAS 프로그램이 종료되면 서블릿 객체를 소멸하기전에  한번만 호출되는 메소드 - 마무리 작업
	@Override
	public void destroy() {
	...
	// => 전달값을 반환받아 데이타를 처리하고 응답 파일 생성하는 명령 작성 - 요청 처리 메소드
