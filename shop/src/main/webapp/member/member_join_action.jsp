...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%-- 회원정보를 전달받아 MEMBER 테이블의 행으로 삽입하고 [/member/member_login.jsp] 문서를 요청할 수있는
    URL 주소로 응답하는 JSP 문서 --%>
<%
	//비정상적으로 JSP 문서를 요청한 경우에 대한 응답처리 
	if(request.getMethod().equals("GET")){ //JSP 문서를 GET 방식으로 요청한 경우
		//[/error/error_400.jsp] 문서를 요청할 수 있는 URL 주소를 전달하여 응답
		// => URL 주소를 클라이언트에게 전달하여 응답하지 않고 요청 JSP 문서 (index.jsp)에게 전달하여 응답
		//하므로 페이지 이동 불가능 
		//response.sendRedirect(request.getContextPath()+"/error/error_400.jsp");
		//return;
		
		//request 내장객체의 속성값으로 에레머세지를 출력하는 페이지의  URL 주소를 저장하여 요청 JSP 문서
	...
	//MemberDTO 객체 생성하여 전달값으로 필드값 변경
	...
	//회원정보(MemberDTO 객체)를 전달받아 MEMBER 테이블의 행으로 삽입하고 삽입행의 갯수를 반환하는 MemberDAO 클래스 호출	
	//반환하는 MemberDAO 클래스 호출
	MemberDAO.getDAO().insertMember(member);
	
	//request 내장객체  속성값 URL 주소 저장하여 요청 JSP 문서(i 에게 제공
			//=> 요청 JSP 문서 (index.jsp)에서 URL ㅁ주소를 반환받아 리다이렉트 이동
