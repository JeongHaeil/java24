<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%-- => [입력완료 ] 태그를 클릭한 경우 전달값에 따라 다른 JSP 문서 요청 - 입력값(비밀번호) 전달 --%>
    ...
    <%	if(action.equals("modify")) { %>
    		<p> 회원정보 변경을 위해 비밀번호 입력 해주세요 </p>
    <% } else {%>
    	<p>회원탈퇴를 위해 비밀번호를 입력해주세요
    <%} %>
<form method="post" id="passwordForm">
	<input type="password" name="passwd" id="passwd">
	<button type="button" id="submitBtn">입력완료</button>
</form>
<p id="message" style="cloor : red;"></p>

<script type="text/javascript">
$("#passwd").focus();
$("#submitBtn").click(function(){
	if($("#passwd").val()==""){
		$("#message").text("비밀번호 입력해주세요");
		return;
	}
	...
	<% } else {  %>
	$("#passwordForm").attr("action", "<%=request.getContextPath()%>/index.jsp?workgroup=member&work=member_action_remove");
	<% } %>
